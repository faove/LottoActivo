...
//if (!is_null($insert)){
	...
	if ($monto<=$monto_max){
		...
		echo "Monto no puede ser mayor que".$monto_max."";
		$varmonto=1;
	...
	if ($varmonto==0){
		...
		//echo "Debo insertar flag_imprimir es: ".$deboinsertar;	
	...
	/*if ($deboinsertar==0 && $varmonto==0){
		...
		$strqueryani = "SELECT animalito,monto_max_9,monto_max_g_9,monto_max_10,monto_max_g_10,monto_max_11,monto_max_g_11,monto_max_12,monto_max_g_12, " .
			"monto_max_1,monto_max_g_1,monto_max_3,monto_max_g_3,monto_max_4,monto_max_g_4,monto_max_5,monto_max_g_5,monto_max_6,monto_max_g_6,monto_max_7,monto_max_g_7 FROM ruleta_activa where num_animalito='".$buscar_txt."'";
		//echo $strqueryani;
		//echo $strqueryani;
		...
		while($rowani = mysqli_fetch_array($resultani)) {  
			...
			//Granjita
			$monto_max_g_9 = $rowani['monto_max_g_9'];
			$monto_max_g_10 = $rowani['monto_max_g_10'];
			$monto_max_g_11 = $rowani['monto_max_g_11'];
			$monto_max_g_12 = $rowani['monto_max_g_12'];
			$monto_max_g_1 = $rowani['monto_max_g_1'];
			$monto_max_g_3 = $rowani['monto_max_g_3'];
			$monto_max_g_4 = $rowani['monto_max_g_4'];
			$monto_max_g_5 = $rowani['monto_max_g_5'];	
			$monto_max_g_6 = $rowani['monto_max_g_6'];	
			$monto_max_g_7 = $rowani['monto_max_g_7'];
			
			
		}
		list($horalist,$descripcionlist) = explode('-',$select_sorteo);
		//echo $select_sorteo;
		include("monto_maximo.php");
		...
		if ($varmonto==0){
			...
			if (!isset($nuevoticket)){
				...
				//echo "esta definida $nuevoticket:".$nuevoticket;
				//$nuevoticket = uniqid();range();
				/*--
				$rand = range(1, 99); 
				shuffle($rand); 
				foreach ($rand as $val) { 
					//echo $val . '<br />'; 
					$num[$nuevoserial]=$val*2;
				}
				--*/
				
				//echo $val . '<br />'; 
				//$nuevoticket= $num[$nuevoserial];
				//--$nuevoticket= $num[$nuevoserial]/$nuevoserial;
				//$nuevoticket = $num[$nuevoserial];
				//$horarand=date(HYimds);
				$nuevoticket=date(HYimds);	
				//$nuevoticket=$nuevoserial*date(s);
			...
			$insert_jug = "INSERT INTO jugadas (Serial,Ticket,Fecha,Sorteo,Loteria,Animalito,Num_animalito, ".
							...
							" values('".$nuevoserial."','".$nuevoticket."',NOW(),'".$horalist."','".$descripcionlist."','".$animalitol."','".$buscar_txt. "','" .$monto. "','" .$total. "'," .$pagar. ",'N',0,0,0,0,0)";
			//printf("%s\n\t");
			//echo $insert_jug;
				...
				if (mysqli_query($con, $insert_jug)) {
					...
					echo "<p><font color=#000080 size=2 face=Arial Narrow>Los datos no han sido INSERTADOS insert animalitos</font></p>";
				}
			}
